...
public class Validator {
    ...
    public static String validate(RequestData data) {
        BigDecimal x = data.x();
        BigDecimal y = data.y();
        BigDecimal r = data.r();

        if (x == null || x.compareTo(X_MIN) < 0 || x.compareTo(X_MAX) > 0) {
            return "x должен быть числом в диапазоне [" + X_MIN + ", " + X_MAX + "]";
        }

        if (y == null || y.compareTo(Y_MIN) < 0 || y.compareTo(Y_MAX) > 0) {
            return "y должен быть числом в диапазоне [" + Y_MIN + ", " + Y_MAX + "]";
        }

        if (r == null || r.compareTo(R_MIN) < 0 || r.compareTo(R_MAX) > 0) {
            return "r должен быть числом в диапазоне [" + R_MIN + ", " + R_MAX + "]";
        }
